...
class Piece:
    ...
    def strip(self):
        if not hasattr(self, 'bbox'):
            cont, (hier, ) = cv.findContours(
                cv.cvtColor(self.img, cv.COLOR_BGR2GRAY), cv.RETR_TREE,
                cv.CHAIN_APPROX_SIMPLE
            )
            center = (int(self.img.shape[0] // 2), int(self.img.shape[1] // 2))
            cond = [
                cv.pointPolygonTest(i, center, False) == 1
                and cv.pointPolygonTest(i, (0, 0), False) == -1 for i in cont
            ]
        ...
        r = self.img[self.bbox[1]:self.bbox[1] + self.bbox[3],
    ...
    def setContour(self, cont: list):
        ...
        self.padding = (
            *self.bbox[:2], self.img.shape[1] - self.bbox[2] - self.bbox[0],
            self.img.shape[0] - self.bbox[3] - self.bbox[1]
        )

    def strip_mask(self):
        r = self.mask[self.bbox[1]:self.bbox[1] + self.bbox[3],
                      self.bbox[0]:self.bbox[0] + self.bbox[2]]
        return r * self.strip().astype('bool')

    def imitated(self):
        spiece = (self.strip() * 0.3).astype('uint8')
        cont = np.array([[[x - self.bbox[0], y - self.bbox[1]]]
                         for (x, y), in self.cont],
                        dtype='int')
...
class Jigsaw:
    ...
    def solve(self) -> int:
        ...
        if debug:
            ...
            cont = [[[x + left - self.piece.padding[0], y + self.top]]
                    for (x, y), in self.piece.cont]
